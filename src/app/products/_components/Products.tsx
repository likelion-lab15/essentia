...
// API 호출 함수 (페이지네이션, 정렬)
const getProducts = async (sortOrder, page, limit) => {
  try {
...
export default function Products({ selectedBrand }) {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(3); // 페이지 항목 수 상태 (임시로 상품 3개)
  const [totalPages, setTotalPages] = useState(10); // 총 페이지 수 (고정 값)
  ...
  const handleNextGroup = () => {
    setPageGroupStart((start) => Math.min(start + PAGES_PER_GROUP, totalPages));
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      const allProducts = await getProducts(null, currentPage, itemsPerPage); // 기본 정렬 사용 (ID 기준)
    ...
    fetchData();
  }, [selectedBrand, currentPage, itemsPerPage]);
    ...
    <div className="w-[984px]">
      ...
      <div className="mt-[100px] flex w-[984px] items-center justify-center">
        ...
        <button
          ...
          disabled={pageGroupStart + PAGES_PER_GROUP > totalPages}
          className="ml-[20px]"
        >
